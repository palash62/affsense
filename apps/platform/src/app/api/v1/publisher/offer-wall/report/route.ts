...
import { listOfferWallReportForPublisher } from "@/services/offer-wall-report.service";
...
export async function GET(request: Request) {
  return withAuth(async (session) => {
    try {
      ...
      const data = await listOfferWallReportForPublisher(session.user.id, parsed.data);
