...
export function PublisherMarketplaceAffiliateReport({
  defaultFrom,
  defaultTo,
}: {
  defaultFrom: string;
  ...
  const load = useCallback(async () => {
    ...
    const res = await fetch(
      `/api/v1/publisher/digital-products/affiliate-product-report?${params}`,
    );
  ...
  return (
    <div className="space-y-6">
      <PageHero
        eyebrow="Marketplace"
        title="Report"
        description="Your product performance — clicks, orders, conversion rate, and EPC."
