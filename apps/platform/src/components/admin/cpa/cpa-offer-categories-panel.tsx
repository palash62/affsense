...
export function CpaOfferCategoriesPanel() {
  const [categories, setCategories] = useState<CpaOfferCategoryItem[]>([]);
  const [name, setName] = useState("");
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editName, setEditName] = useState("");

  async function refresh() {
    const res = await fetch("/api/v1/admin/cpa-offers/categories");
  ...
  async function handleDelete(id: string) {
    const res = await fetch(`/api/v1/admin/cpa-offers/categories?id=${id}`, {
      method: "DELETE",
    });
    if (!res.ok) {
      const json = await res.json().catch(() => ({}));
    ...
    <div className="space-y-5">
      ...
      <DashboardCard className="overflow-hidden p-0">
        <div className="border-b border-border px-5 py-4">
          <DashboardCardTitle>CPA Offer Categories</DashboardCardTitle>
            ...
            <tbody>
              {categories.map((cat) => (
                <tr key={cat.id} className="border-t border-border">
                  ...
                  <td className="px-5 py-3 text-muted-foreground">{cat.offerCount}</td>
